...
static const List<Widget> _widgetOptions = <Widget>[
  ...
  Widget build(BuildContext context) {
    ...
    BlocProvider.of<ChatCubit>(context).getChat(context,context.read<LoginCubit>().state.userData?['uid']);
    BlocProvider.of<NotificationCubit>(context).getNotifications(context,context.read<LoginCubit>().state.userData?['uid']);
    BlocProvider.of<DashboardCubit>(context).getAllOrders(context, context.read<LoginCubit>().state.userData?['uid']);
    ...
    return BlocBuilder<DashboardCubit,DashboardState>(builder: (context, state) {
      Map<String, dynamic>? userData = context.watch<LoginCubit>().state.userData;
      return Scaffold(
        backgroundColor: kWhite,
        drawer: const CustomDrawer(),
        appBar: AppBar(
          backgroundColor: kWhite,
          title: Text(dashboardAppbarNamer(state,context),style: const TextStyle(color: kBlack),),
          centerTitle: true,
          elevation: 0, 
          toolbarHeight: 50,
          actions: [
            ...
            }, icon: const Icon(Icons.notifications_rounded, color: kBlack,))
          ],
          ...
          ? PreferredSize(
            ...
            child: Container(
              ...
              child: Row(
                children: [
                  const Expanded(child: CustomText(text: 'To publish your shop please register your shop',textAlign: TextAlign.left,fontsize: 13,)),
          ...
          ? PreferredSize(
            ...
            child: Container(
              ...
              child: const Row(
                children: [
                  Expanded(child: CustomText(text: 'Thanks for your registration your store sent for approval please stay patient we\'ll reach with in 2-3 days',textAlign: TextAlign.left,fontsize: 13,)),
                ],
              ),
            ),
          )
          :PreferredSize(preferredSize: const Size(double.maxFinite, 0),child: Container(),),
        ),
...
String dashboardAppbarNamer(DashboardState state,BuildContext context) => (state.currentScreenIndex==0)?'${context.read<LoginCubit>().state.userData?["shopName"] ?? ""}':(state.currentScreenIndex==1)?'Feeds':(state.currentScreenIndex==2)?'Support':(state.currentScreenIndex==3)?'Profile':'Dastan-e-Hunar';
  List<BottomNavigationBarItem> _navBarsItems() {
        return [
            const BottomNavigationBarItem(
                icon: Icon(CupertinoIcons.home),
                activeIcon: Icon(CupertinoIcons.house_fill, color: primaryColor,),
                label: 'Home'
            ),
             const BottomNavigationBarItem(
                icon: Icon(CupertinoIcons.flame,),
               activeIcon: Icon(CupertinoIcons.flame_fill, color: primaryColor,),
                label: 'Feeds'
            ), 
            const BottomNavigationBarItem(
                icon: Icon(CupertinoIcons.chat_bubble_text,),
                label: "Chats",
                activeIcon: Icon(CupertinoIcons.chat_bubble_text_fill, color: primaryColor,),
            ), 
            const BottomNavigationBarItem(
                icon: Icon(CupertinoIcons.person),
                label: "Profile",
                activeIcon: Icon(CupertinoIcons.person_fill, color: primaryColor,),
            ),
        ];
    }
